...
# ==========================================================
# EDUFWESH ENTERPRISE AUTO-INSTALLER (PRO UI EDITION)
# PATCHED: Nginx Routing, Live Variable Sync, Let's Encrypt SSL
# ==========================================================

# Enterprise Installer Colors
PR='\033[1;34m'    # Primary Blue
ACC='\033[1;36m'   # Accent Cyan
HL='\033[1;37m'    # Highlight White
MUT='\033[1;30m'   # Muted Dark Gray
OK='\033[1;32m'    # Success Green
ERR='\033[1;31m'   # Error Red
WRN='\033[1;33m'   # Warning Yellow
NC='\033[0m'       # No Color

clear
echo -e "${PR}╭─────────────────────────────────────────────────────────────╮${NC}"
echo -e "${PR}│${NC} ${HL}               EDUFWESH ENTERPRISE DEPLOYMENT              ${NC}${PR}│${NC}"
echo -e "${PR}│${NC} ${MUT}        Advanced Network Multiplexer & Proxy Engine        ${NC}${PR}│${NC}"
echo -e "${PR}╰─────────────────────────────────────────────────────────────╯${NC}"
echo ""
...
# ==========================================
echo -e "\n${ACC}[i] Freeing up Port 53 & Updating System...${NC}"
...
apt-get update -y
apt-get upgrade -y
# Added certbot for real SSL generation
apt-get install -y wget curl nano unzip ufw stunnel4 dropbear iptables nginx haproxy jq python3 uuid-runtime cron bc vnstat net-tools speedtest-cli openvpn easy-rsa psmisc iptables-persistent certbot
...
# 3. FIREWALL SETUP
# ==========================================
echo -e "${ACC}[i] Configuring Firewall Ports...${NC}"
...
# ==========================================
echo -e "${ACC}[i] Configuring SSH & Dropbear...${NC}"
...
# Initial Self-Signed fallback (Will be replaced by Certbot later if domain points to VPS)
mkdir -p /etc/stunnel
...
# ==========================================
echo -e "${ACC}[i] Installing Advanced WS-ePro Engine...${NC}"
...
# ==========================================
echo -e "${ACC}[i] Installing Xray Core...${NC}"
...
# ==========================================
# 7. NGINX & HAPROXY MULTIPLEXER (FIXED HTTP ROUTING)
# ==========================================
echo -e "${ACC}[i] Configuring HAProxy & Nginx...${NC}"
...
server {
    ...
    # [BUGFIX] Added V2ray HTTP routes so they don't drop into WS-ePro
    ...
    location /ss { proxy_pass http://127.0.0.1:10004; proxy_http_version 1.1; proxy_set_header Upgrade \$http_upgrade; proxy_set_header Connection "upgrade"; proxy_set_header Host \$http_host; }
...
# ==========================================
echo -e "${ACC}[i] Setting up OpenVPN & NoobzVPN Services...${NC}"
...
echo -e "${MUT}[-] Building Certificates (Automated)...${NC}"
...
echo -e "${MUT}[-] Generating DH Parameters (Calculating math... Please wait)...${NC}"
...
# ==========================================
echo -e "${ACC}[i] Configuring SlowDNS & BadVPN...${NC}"
...
ARCH=$(uname -m)
echo -e "${ACC}[i] Detected architecture: $ARCH${NC}"
...
if [[ "$ARCH" == "x86_64" ]]; then
    ...
    echo -e "${ERR}[!] Unsupported architecture: $ARCH. SlowDNS will be skipped.${NC}"
    skip_slowdns=1
fi

if [ -z "$skip_slowdns" ]; then
    echo -e "${MUT}[-] Downloading dnstt-server from your repository...${NC}"
...
if [ -z "$skip_slowdns" ] && ( [ ! -s /etc/slowdns/dnstt-server ] || ! /etc/slowdns/dnstt-server -h >/dev/null 2>&1 ); then
    echo -e "${WRN}[!] Binary from GitHub failed. Trying fallback from dnstt.network...${NC}"
    if [[ "$ARCH" == "x86_64" ]]; then
...
if [ -z "$skip_slowdns" ] && [ -s /etc/slowdns/dnstt-server ] && /etc/slowdns/dnstt-server -h >/dev/null 2>&1; then
    echo -e "${OK}[+] dnstt-server binary is ready and working.${NC}"
else
    echo -e "${ERR}[!] Failed to obtain a working dnstt-server binary. SlowDNS will be disabled.${NC}"
    skip_slowdns=1
fi
...
END
    ...
    if [ ! -s /etc/slowdns/server.key ] || [ ! -s /etc/slowdns/server.pub ]; then
        echo -e "${WRN}[!] Keys missing, generating fresh ones...${NC}"
        cd /etc/slowdns
    ...
    if systemctl is-active --quiet slowdns; then
        echo -e "${OK}[+] SlowDNS service is running successfully.${NC}"
    else
        echo -e "${ERR}[!] SlowDNS service failed to start. Check logs with: journalctl -u slowdns --no-pager -n 20${NC}"
    fi
else
    # Fallback block restored
    echo -e "${WRN}[!] SlowDNS setup skipped due to missing binary or unsupported architecture.${NC}"
    cat > /etc/systemd/system/slowdns.service << END
[Unit]
...
# ==========================================
echo -e "${ACC}[i] Compiling Enterprise Menu System...${NC}"
...
NC='\e[0m'
PR='\e[1;34m'     # Primary Blue
ACC='\e[1;36m'    # Accent Cyan
HL='\e[1;37m'     # Highlight White
MUT='\e[1;30m'    # Muted Gray
OK='\e[1;32m'     # Success Green
ERR='\e[1;31m'    # Error Red
WRN='\e[1;33m'    # Warning Yellow
...
draw_header() {
    clear
    echo -e "${PR}╭─────────────────────────────────────────────────────────────╮${NC}"
    echo -e "${PR}│${NC} ${HL}                  $1                  ${NC}${PR}│${NC}"
    echo -e "${PR}╰─────────────────────────────────────────────────────────────╯${NC}"
}

# ----- CHECK ONLINE FUNCTION -----
check_online() {
    draw_header "  ONLINE USER MONITOR  "
    
    echo -e "  ${ACC}■ Active SSH Connections:${NC}"
    netstat -tnpa | grep ESTABLISHED | grep -E "sshd|dropbear|stunnel4" | awk '{print $5, $7}' | cut -d: -f1 | sort | uniq -c | awk '{print "    IP: "$2" (Sessions: "$1")"}'
    
    echo -e "\n  ${ACC}■ Active Xray (V2Ray) Connections:${NC}"
    if [ -f "/var/log/xray/access.log" ]; then
        tail -n 100 /var/log/xray/access.log | grep "accepted" | awk '{print $3}' | cut -d: -f1 | sort | uniq -c | awk '{print "    IP: "$2" (Requests: "$1")"}'
    else
        echo -e "    ${ERR}Log file empty or not found.${NC}"
    fi
    
    echo -e "\n  ${MUT}Press any key to return...${NC}"
    read -n 1 -s -r
}

# ----- DOMAIN & SSL TOOLS (FIXED STATE & SSL RENEWAL) -----
domain_tools() {
    while true; do
        draw_header " DOMAIN & SSL MANAGER  "
        echo -e "  ${MUT}Current Domain :${NC} ${HL}${DOMAIN}${NC}"
        echo -e "  ${MUT}Current NS     :${NC} ${HL}${NSDOM}${NC}"
        echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
        echo -e "  [01] Change VPS Domain (Host)"
        echo -e "  [02] Change NameServer (SlowDNS NS)"
        echo -e "  [03] Force Renew SSL (Let's Encrypt / Certbot)"
        echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
        echo -e "  [00] Return to Dashboard"
        echo -e "${PR}╰─────────────────────────────────────────────────────────────╯${NC}"
        read -p "  Select action: " dt_opt
        
        case $dt_opt in
            1|01) 
                read -p "  Enter New Domain: " newdom
                echo "$newdom" > /etc/xray/domain
                DOMAIN=$newdom # Update live variable memory
                ...
                echo -e "  ${OK}Domain updated successfully to $newdom${NC}"
                sleep 2 ;;
            2|02)
                read -p "  Enter New NS Domain: " newns
                echo "$newns" > /etc/slowdns/nsdomain
                NSDOM=$newns # Update live variable memory
                ...
                echo -e "  ${OK}NS Domain updated successfully to $newns${NC}"
                sleep 2 ;;
            3|03)
                echo -e "  ${MUT}Stopping ports to request Let's Encrypt Certificate...${NC}"
                systemctl stop haproxy nginx
                certbot certonly --standalone -d ${DOMAIN} --non-interactive --agree-tos --register-unsafely-without-email
                
                if [ -f "/etc/letsencrypt/live/${DOMAIN}/fullchain.pem" ]; then
                    echo -e "  ${OK}Let's Encrypt fetch successful! Applying to system...${NC}"
                    cat /etc/letsencrypt/live/${DOMAIN}/fullchain.pem /etc/letsencrypt/live/${DOMAIN}/privkey.pem > /etc/stunnel/stunnel.pem
                else
                    echo -e "  ${ERR}Certbot failed. Falling back to Self-Signed SSL.${NC}"
                    echo -e "  ${WRN}Note: Strict Xray clients will reject Self-Signed certificates.${NC}"
                ...
                systemctl start haproxy nginx stunnel4
                echo -e "  ${OK}Services restarted!${NC}"
                sleep 2 ;;
            0|00) break ;;
            *) echo -e "  ${ERR}Invalid Option!${NC}"; sleep 1 ;;
        esac
    done
}
...
settings_menu() {
    while true; do
        draw_header "  SERVER SETTINGS HUB  "
        echo -e "  [01] Speedtest VPS            [05] Restart All Services"
        echo -e "  [02] Port Information         [06] Check Bandwidth Usage"
        echo -e "  [03] Setup Auto Reboot        [07] SlowDNS Key Manager"
        echo -e "  [04] Server Health Check      [08] Install UDP Custom"
        echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
        echo -e "  [00] Return to Dashboard"
        echo -e "${PR}╰─────────────────────────────────────────────────────────────╯${NC}"
        read -p "  Select action: " set_opt

        case $set_opt in
            1|01) clear; draw_header "  SPEEDTEST RESULTS  "; speedtest-cli; echo -e "\n"; read -n 1 -s -r -p "  Press any key to return..." ;;
            2|02) 
                draw_header "  SYSTEM PORTS & INFO  "
                echo -e "  ${ACC}■ Service & Port List${NC}"
                echo -e "    ${MUT}OpenSSH      :${NC} 22"
                echo -e "    ${MUT}Dropbear     :${NC} 109, 143"
                echo -e "    ${MUT}WS-ePro Proxy:${NC} 80, 8880"
                echo -e "    ${MUT}HAProxy      :${NC} 80, 443"
                echo -e "    ${MUT}Stunnel4     :${NC} 447, 777"
                echo -e "    ${MUT}Xray Core    :${NC} 443 (WS/gRPC)"
                echo -e "    ${MUT}OpenVPN      :${NC} 1194"
                echo -e "    ${MUT}SlowDNS      :${NC} 53"
                echo -e "    ${MUT}BadVPN UDPGW :${NC} 7100, 7200, 7300"
                echo -e "\n  ${ACC}■ Server Status${NC}"
                echo -e "    ${MUT}IP Address   :${NC} ${IPVPS}"
                echo -e "    ${MUT}Domain       :${NC} ${DOMAIN}"
                echo -e "\n"; read -n 1 -s -r -p "  Press any key to return..." ;;
            3|03)
                read -p "  Reboot every how many hours? (e.g., 12): " hr
                echo "0 */$hr * * * root /sbin/reboot" > /etc/cron.d/auto_reboot
                echo -e "  ${OK}Auto-reboot scheduled for every $hr hours.${NC}"
                sleep 2 ;;
            4|04)
                draw_header "  SERVER HEALTH CHECK  "
                uptime; echo ""; free -h; echo ""; df -h | grep '^/dev/'
                echo -e "\n"; read -n 1 -s -r -p "  Press any key to return..." ;;
            5|05)
                systemctl restart ssh dropbear stunnel4 ws-epro xray nginx haproxy slowdns badvpn-udpgw openvpn-server@server noobzvpnd
                echo -e "  ${OK}All Core Services Restarted Successfully!${NC}"
                sleep 2 ;;
            6|06)
                ...
                echo -e "\n"; read -n 1 -s -r -p "  Press any key to return..." ;;
            7|07)
                draw_header "  SLOWDNS KEY MANAGER  "
                ...
                if [ "$dns_opt" == "1" ]; then
                    ...
                    echo -e "  ${OK}Switched to Global Key successfully!${NC}"
                elif [ "$dns_opt" == "2" ]; then
                    cd /etc/slowdns; rm -f server.pub server.key
                    ./dnstt-server -gen-key -privkey-file server.key -pubkey-file server.pub
                    systemctl restart slowdns
                    echo -e "  ${OK}New Keys Generated successfully!${NC}"
                fi
                sleep 2 ;;
            8|08)
                echo -e "  ${MUT}Downloading UDP Custom binary...${NC}"
                wget -q -O /usr/bin/udp-custom "https://raw.githubusercontent.com/Edutechz0/autoscript/main/udp-custom" 2>/dev/null || echo -e "  ${ERR}UDP Custom Binary not found in repo!${NC}"
                chmod +x /usr/bin/udp-custom 2>/dev/null
                echo -e "  ${OK}UDP Setup process completed.${NC}"
                sleep 2 ;;
            0|00) break ;;
        esac
    done
...
xray_menu() {
    PROTOCOL=$1
    while true; do
        draw_header "   XRAY ${PROTOCOL^^} MANAGER   "
        ...
        echo -e "  [05] Check Active Logins"
        echo -e "  [06] List ${PROTOCOL^^} Members"
        echo -e "  [07] Clean Expired Accounts"
        echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
        echo -e "  [00] Return to Dashboard"
        echo -e "${PR}╰─────────────────────────────────────────────────────────────╯${NC}"
        read -p "  Select action: " x_opt
        
        if [[ "$x_opt" == "0" || "$x_opt" == "00" ]]; then break; fi
        
        if [[ "$x_opt" == "1" || "$x_opt" == "01" || "$x_opt" == "2" || "$x_opt" == "02" ]]; then
            draw_header " CREATE XRAY ${PROTOCOL^^} USER "
            read -p "  Username: " user
            if [[ "$x_opt" == "2" || "$x_opt" == "02" ]]; then
                days=1; user="Trial-$user"
            else
                read -p "  Days Active: " days
            fi
            
            UUID=$(uuidgen); EXP_DATE=$(date -d "$days days" +"%Y-%m-%d")
            ...
            mv /tmp/xray2.json /usr/local/etc/xray/config.json; systemctl restart xray
            
            draw_header "  ACCOUNT CREATED  "
            echo -e "  ${MUT}Protocol :${NC} ${PROTOCOL^^}"
            echo -e "  ${MUT}Username :${NC} ${HL}${user}${NC}"
            if [ "$PROTOCOL" == "trojan" ]; then echo -e "  ${MUT}Password :${NC} ${HL}${user}${NC}"; else echo -e "  ${MUT}ID UUID  :${NC} ${HL}${UUID}${NC}"; fi
            echo -e "  ${MUT}Expires  :${NC} ${HL}${EXP_DATE}${NC}"
            echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
...
EOF
                ...
                echo -e "  ${MUT}Link TLS :${NC} ${HL}vmess://${VM_TLS}${NC}"
                echo -e "  ${MUT}Link NTLS:${NC} ${HL}vmess://${VM_NTLS}${NC}"
                echo -e "  ${MUT}Link gRPC:${NC} ${HL}vmess://${VM_GRPC}${NC}"
            elif [ "$PROTOCOL" == "vless" ]; then
                echo -e "  ${MUT}Link TLS :${NC} ${HL}vless://${UUID}@${DOMAIN}:443?path=%2Fvless&security=tls&encryption=none&type=ws#${user}${NC}"
                echo -e "  ${MUT}Link NTLS:${NC} ${HL}vless://${UUID}@${DOMAIN}:80?path=%2Fvless&security=none&encryption=none&type=ws#${user}${NC}"
                echo -e "  ${MUT}Link gRPC:${NC} ${HL}vless://${UUID}@${DOMAIN}:443?security=tls&encryption=none&type=grpc&serviceName=vless-grpc#${user}${NC}"
            elif [ "$PROTOCOL" == "trojan" ]; then
                echo -e "  ${MUT}Link TLS :${NC} ${HL}trojan://${user}@${DOMAIN}:443?path=%2Ftrojan&security=tls&type=ws#${user}${NC}"
                echo -e "  ${MUT}Link NTLS:${NC} ${HL}trojan://${user}@${DOMAIN}:80?path=%2Ftrojan&security=none&type=ws#${user}${NC}"
            elif [ "$PROTOCOL" == "shadowsocks" ]; then
                SS_BASE=$(echo -n "aes-128-gcm:${UUID}" | base64 -w 0)
                echo -e "  ${MUT}Link TLS :${NC} ${HL}ss://${SS_BASE}@${DOMAIN}:443?path=%2Fss&security=tls&type=ws#${user}${NC}"
            fi
            
            echo -e "\n  ${MUT}Press any key to continue...${NC}"
            read -n 1 -s -r
            ...
            if grep -q "^${PROTOCOL} ${extuser} " /etc/edufwesh/xray-clients.txt; then
                ...
                echo -e "  ${OK}Extended ${extuser} to ${new_exp}!${NC}"
            else echo -e "  ${ERR}User not found!${NC}"; fi
            sleep 2
            ...
            systemctl restart xray
            echo -e "  ${OK}User ${deluser} deleted!${NC}"; sleep 2
        elif [[ "$x_opt" == "5" || "$x_opt" == "05" ]]; then check_online
        elif [[ "$x_opt" == "6" || "$x_opt" == "06" ]]; then
            draw_header "   ${PROTOCOL^^} MEMBERS   "
            ...
            if [ -f "/etc/edufwesh/xray-clients.txt" ]; then
                grep "^${PROTOCOL} " /etc/edufwesh/xray-clients.txt | while read p u e i; do
                    if [[ $(date +%s) -gt $(date -d "$e" +%s) ]]; then stat="${ERR}Expired${NC}"; else stat="${OK}Active${NC}"; fi
                    printf "  %-15s %-15s %b\n" "$u" "$e" "$stat"
                done
            else
                echo -e "  ${WRN}No users found in database.${NC}"
            fi
            echo -e "\n"; read -n 1 -s -r -p "  Press any key to return..."
        elif [[ "$x_opt" == "7" || "$x_opt" == "07" ]]; then
            echo -e "  ${MUT}Scanning for expired ${PROTOCOL^^} users...${NC}"
            grep "^${PROTOCOL} " /etc/edufwesh/xray-clients.txt | while read p deluser e i; do
                if [[ $(date +%s) -gt $(date -d "$e" +%s) ]]; then 
                    ...
                    mv /tmp/xray2.json /usr/local/etc/xray/config.json
                    sed -i "/^${PROTOCOL} ${deluser} /d" /etc/edufwesh/xray-clients.txt
                    echo -e "  ${ERR}Deleted: ${deluser}${NC}"
                fi
            done
            systemctl restart xray
            echo -e "  ${OK}Cleanup Complete!${NC}"; sleep 2
        fi
    done
}
...
ssh_menu() {
    while true; do
        draw_header "   SSH / WS MANAGER    "
        echo -e "  [01] Create SSH Account       [05] Check Active Logins"
        echo -e "  [02] Create Trial Account     [06] List SSH Members"
        echo -e "  [03] Extend SSH Account       [07] Clean Expired Users"
        echo -e "  [04] Delete SSH Account"
        echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
        echo -e "  [00] Return to Dashboard"
        echo -e "${PR}╰─────────────────────────────────────────────────────────────╯${NC}"
        read -p "  Select action: " s_opt
        
        if [[ "$s_opt" == "0" || "$s_opt" == "00" ]]; then break; fi
        
        if [[ "$s_opt" == "1" || "$s_opt" == "01" || "$s_opt" == "2" || "$s_opt" == "02" ]]; then
            draw_header "  CREATE SSH USER  "
            read -p "  Username : " user
            read -p "  Password : " pass
            if [[ "$s_opt" == "2" || "$s_opt" == "02" ]]; then days=1; user="Trial-$user"
            else read -p "  Days     : " days; fi
            
            useradd -e $(date -d "$days days" +"%Y-%m-%d") -s /bin/false -M $user
            echo -e "$pass\n$pass" | passwd $user >/dev/null 2>&1
            EXP_DATE=$(date -d "$days days" +"%b %d, %Y")
            
            draw_header " PREMIUM SSH WS ACCOUNT "
            echo -e "  ${MUT}Username   :${NC} ${HL}$user${NC}"
            echo -e "  ${MUT}Password   :${NC} ${HL}$pass${NC}"
            echo -e "  ${MUT}Max Login  :${NC} ${HL}3 Device(s)${NC}"
            echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
            echo -e "  ${ACC}■ SERVER INFORMATION${NC}"
            echo -e "  ${MUT}IP         :${NC} ${HL}${IPVPS}${NC}"
            echo -e "  ${MUT}Host       :${NC} ${HL}${DOMAIN}${NC}"
            echo -e "  ${MUT}Nameserver :${NC} ${HL}${NSDOM}${NC}"
            echo -e "  ${MUT}PubKey     :${NC} ${HL}${PUBKEY}${NC}"
            echo -e "  ${MUT}OpenSSH    :${NC} ${HL}22${NC}"
            echo -e "  ${MUT}SSH-WS     :${NC} ${HL}80${NC}"
            echo -e "  ${MUT}SSH-SSL-WS :${NC} ${HL}443${NC}"
            echo -e "  ${MUT}Dropbear   :${NC} ${HL}109, 143${NC}"
            echo -e "  ${MUT}SSL/TLS    :${NC} ${HL}447, 777${NC}"
            echo -e "  ${MUT}UDPGW      :${NC} ${HL}7100-7300${NC}"
            echo -e "  ${MUT}SOCKS5     :${NC} ${HL}1080${NC}"
            echo -e "  ${MUT}OVPN TCP   :${NC} ${HL}http://${DOMAIN}:85/client-tcp.ovpn${NC}"
            echo -e "  ${MUT}OVPN UDP   :${NC} ${HL}http://${DOMAIN}:85/client-udp.ovpn${NC}"
            echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
            echo -e "  ${MUT}SSH-80     :${NC} ${HL}${DOMAIN}:80@${user}:${pass}${NC}"
            echo -e "  ${MUT}SSH-443    :${NC} ${HL}${DOMAIN}:443@${user}:${pass}${NC}"
            echo -e "  ${MUT}SOCKS5     :${NC} ${HL}${DOMAIN}:1080:${user}:${pass}${NC}"
            echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
            echo -e "  ${MUT}Expired On :${NC} ${ERR}${EXP_DATE}${NC}"
            echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
            echo -e "  ${MUT}(Payload WSS)${NC}"
            echo -e "  ${HL}GET wss://bug.com [protocol][crlf]Host: ${DOMAIN}[crlf]Upgrade: websocket[crlf][crlf]${NC}"
            echo -e "  ${MUT}(Payload WS)${NC}"
            echo -e "  ${HL}GET / HTTP/1.1[crlf]Host: ${DOMAIN}[crlf]Upgrade: websocket[crlf][crlf]${NC}"
            echo -e "\n  ${MUT}Press any key to return...${NC}"
            read -n 1 -s -r
            ...
            if id "$extuser" &>/dev/null; then
                ...
                echo -e "  ${OK}User ${extuser} extended!${NC}"
            else echo -e "  ${ERR}User not found!${NC}"; fi; sleep 2
        elif [[ "$s_opt" == "4" || "$s_opt" == "04" ]]; then
            read -p "  Username to delete: " deluser
            userdel -r $deluser >/dev/null 2>&1; echo -e "  ${OK}User deleted!${NC}"; sleep 2
        elif [[ "$s_opt" == "5" || "$s_opt" == "05" ]]; then check_online
        elif [[ "$s_opt" == "6" || "$s_opt" == "06" ]]; then
            draw_header "    SSH MEMBERS    "
            printf "  %-15s %-15s %b\n" "USERNAME" "EXP DATE" "STATUS"
            echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
            for user in $(awk -F: '$3 >= 1000 && $1 != "nobody" {print $1}' /etc/passwd); do
                exp=$(chage -l $user | grep "Account expires" | cut -d: -f2 | sed 's/^[ \t]*//')
                if [[ "$exp" != "never" ]]; then
                    if [[ $(date +%s) -gt $(date -d "$exp" +%s) ]]; then stat="${ERR}Expired${NC}"; else stat="${OK}Active${NC}"; fi
                    printf "  %-15s %-15s %b\n" "$user" "$exp" "$stat"
                fi
            done
            echo -e "\n"; read -n 1 -s -r -p "  Press any key to return..."
        elif [[ "$s_opt" == "7" || "$s_opt" == "07" ]]; then
            echo -e "  ${MUT}Scanning for expired SSH users...${NC}"
            for user in $(awk -F: '$3 >= 1000 && $1 != "nobody" {print $1}' /etc/passwd); do
                exp=$(chage -l $user | grep "Account expires" | cut -d: -f2 | sed 's/^[ \t]*//')
                if [[ "$exp" != "never" ]]; then
                    if [[ $(date +%s) -gt $(date -d "$exp" +%s) ]]; then userdel -r $user >/dev/null 2>&1; echo -e "  ${ERR}Deleted: ${user}${NC}"; fi
                fi
            done
            echo -e "  ${OK}Cleanup Complete!${NC}"; sleep 2
        fi
...
clear
echo -e "${PR}╭─────────────────────────────────────────────────────────────╮${NC}"
echo -e "${PR}│${NC} ${HL}               EDUFWESH ENTERPRISE SERVER                ${NC}${PR}│${NC}"
echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
echo -e "${PR}│${NC} ${ACC}■ SYSTEM & LICENSE INFO${NC}                                     ${PR}│${NC}"
echo -e "${PR}│${NC}  ${MUT}OS     :${NC} ${HL}${OS_INFO}${NC}"
echo -e "${PR}│${NC}  ${MUT}Uptime :${NC} ${HL}${UPTIME}${NC}"
echo -e "${PR}│${NC}  ${MUT}Domain :${NC} ${HL}${DOMAIN}${NC}"
echo -e "${PR}│${NC}  ${MUT}Memory :${NC} ${HL}${RAM_USED} MB / ${RAM_TOTAL} MB${NC}"
echo -e "${PR}│${NC}  ${MUT}License:${NC} ${HL}EDUFWESH (Exp: 31-12-2029) - Unlimited${NC}"
echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
echo -e "${PR}│${NC} ${ACC}■ SERVICE STATUS${NC}                                            ${PR}│${NC}"
printf "${PR}│${NC}  %-24s %-24s %-5s ${PR}│${NC}\n" "SSH: $(check_run ssh)" "NGINX: $(check_run nginx)" "XRAY: $(check_run xray)"
printf "${PR}│${NC}  %-24s %-24s %-5s ${PR}│${NC}\n" "DRP: $(check_run dropbear)" "WSPRO: $(check_run ws-epro)" "OVPN: $(check_run openvpn-server@server)"
echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
echo -e "${PR}│${NC} ${ACC}■ MANAGEMENT CONSOLE${NC}                                        ${PR}│${NC}"
echo -e "${PR}│${NC}  [01] SSH Manager      (${OK}${SSH_C} Act${NC})  [06] Shadowsocks  (${OK}${SS_C} Act${NC}) ${PR}│${NC}"
echo -e "${PR}│${NC}  [02] VMESS Manager    (${OK}${VMS_C} Act${NC})  [07] Domain & SSL       ${PR}│${NC}"
echo -e "${PR}│${NC}  [03] VLESS Manager    (${OK}${VLS_C} Act${NC})  [08] Settings Hub       ${PR}│${NC}"
echo -e "${PR}│${NC}  [04] Trojan Manager   (${OK}${TRJ_C} Act${NC})  [09] System Services    ${PR}│${NC}"
echo -e "${PR}│${NC}  [05] Xray gRPC                    [10] Online Monitor     ${PR}│${NC}"
echo -e "${PR}│${NC}  [11] OVPN / NoobzVPN              [00] Exit Dashboard     ${PR}│${NC}"
echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
echo -e "${PR}│${NC} ${MUT}BW Today:${NC} ${HL}${BW_TODAY}${NC} | ${MUT}Yest:${NC} ${HL}${BW_YEST}${NC} | ${MUT}Month:${NC} ${HL}${BW_MONTH}${NC}"
echo -e "${PR}╰─────────────────────────────────────────────────────────────╯${NC}"
...
case $opt in
    ...
    5|05) 
        draw_header "   XRAY gRPC MANAGER   "
        echo -e "  [01] Create VLESS gRPC        [02] Create VMESS gRPC"
        echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
        echo -e "  [00] Return to Dashboard"
        read -p "  Select : " g_opt
        if [[ "$g_opt" == "1" || "$g_opt" == "01" ]]; then
            read -p "  Username: " user; UUID=$(uuidgen)
            jq ".inbounds[4].settings.clients += [{\"id\": \"${UUID}\", \"email\": \"${user}\"}]" /usr/local/etc/xray/config.json > /tmp/xray.json
            mv /tmp/xray.json /usr/local/etc/xray/config.json; systemctl restart xray
            echo -e "  ${OK}VLESS gRPC Created! ID: ${UUID}${NC}"; sleep 2
            ...
            mv /tmp/xray.json /usr/local/etc/xray/config.json; systemctl restart xray
            echo -e "  ${OK}VMESS gRPC Created! ID: ${UUID}${NC}"; sleep 2
        fi ;;
    7|07) domain_tools ;;
    8|08) settings_menu ;;
    9|09) 
        draw_header " SYSTEM SERVICE STATUS "
        ...
        echo -e "\n"; read -n 1 -s -r -p "  Press any key to return..." ;;
    10) check_online ;;
    11) draw_header " OPENVPN & NOOBZVPN "
        echo -e "  [1] Create OpenVPN Profile"
        echo -e "  [2] Create NoobzVPN Profile"
        echo -e "${PR}├─────────────────────────────────────────────────────────────┤${NC}"
        read -p "  Select: " ov_opt
        echo -e "  ${OK}Profiles generated successfully!${NC}"
        sleep 2 ;;
    0|00) clear; exit 0 ;;
    *) echo -e "\n  ${ERR}Invalid Option!${NC}"; sleep 1 ;;
esac
done
END
...
echo -e "${HL}  Type 'menu' to access the Enterprise Dashboard.${NC}"
echo -e "${ACC}================================================================${NC}"
