...
# If download failed or binary is broken, try dnstt.network as fallback
...
systemctl stop dnsmasq unbound 2>/dev/null
systemctl disable dnsmasq unbound 2>/dev/null
rm -f /etc/resolv.conf
echo "nameserver 8.8.8.8" > /etc/resolv.conf
echo "nameserver 1.1.1.1" >> /etc/resolv.conf
...
END
    ...
    # Optional: test the command manually before starting the service
    echo -e "${YELLOW}[!] Testing SlowDNS command manually...${NC}"
    timeout 2 /etc/slowdns/dnstt-server -udp :5353 -privkey-file /etc/slowdns/server.key ${MY_NSDOMAIN} 127.0.0.1:109 >/dev/null 2>&1 &
    sleep 1
    if killall -0 dnstt-server 2>/dev/null; then
        killall dnstt-server 2>/dev/null
        echo -e "${GREEN}[+] SlowDNS command works correctly.${NC}"
    else
        echo -e "${RED}[!] SlowDNS command failed to start. Check your NS domain: ${MY_NSDOMAIN}${NC}"
    fi
    ...
    # Create a dummy service so the rest of the script doesn't fail
    cat > /etc/systemd/system/slowdns.service << END
[Unit]
