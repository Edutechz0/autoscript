...
# ==========================================
# 7. NGINX & HAPROXY MULTIPLEXER (FIXED WEBSOCKET)
# ==========================================
    ...
    while true; do
        ...
        case $set_opt in
            ...
            9|09)
                ...
                echo -e " Testing WebSocket paths on port 443 (TLS)..."
                echo -e ""
                for path in vmess vless trojan ss; do
                    echo -n " $path: "
                    # Use verbose to capture first line of response
                    response=$(curl -sk --max-time 5 -i "https://${DOMAIN}/${path}" \
                        --header "Host: ${DOMAIN}" \
                        --header "Upgrade: websocket" \
                        --header "Connection: Upgrade" \
                        --header "Sec-WebSocket-Key: x3JJHMbDL1EzLkh9GBhXDw==" \
                        --header "Sec-WebSocket-Version: 13" 2>/dev/null | head -n1)
                    echo "$response"
                done
                echo -e "${CYAN}───────────────────────────────────────────────────────${NC}"
                echo -e " If you see '101 Switching Protocols', the service is working."
                echo -e " If you see '400 Bad Request' or '404 Not Found':"
                echo -e "   1. Ensure you have created at least one account for that protocol (via VMESS/VLESS/Trojan/SS menu)."
                echo -e "   2. Check Xray error log: tail -f /var/log/xray/error.log"
                echo -e "   3. Restart services: systemctl restart xray nginx haproxy"
                echo -e ""
