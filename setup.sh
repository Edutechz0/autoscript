...
ufw disable
# Only opening external-facing ports. Internal proxy ports stay closed to the outside.
...
# 8. NGINX MULTIPLEXER (The Master Router on Port 443)
...
server {
    ...
    # Fake Dummy Page for Scanners
    location / {
        proxy_pass https://www.microsoft.com;
    }

    # Route to Python WS (SSH)
    location /edufwesh-ssh {
        proxy_redirect off;
...
case $opt in
    1|01)
        ...
        echo -e "Host   : $DOMAIN"
        echo -e "Port   : 443 (WSS), 80 (WS)"
        echo -e "Path   : /edufwesh-ssh"
        echo -e "User   : $user"
        echo -e "Pass   : $pass"
        ;;
    2|02)
