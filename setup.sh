...
# ==========================================
# 6. XRAY CORE (VLESS, VMESS, TROJAN, SS + SOCKS5) With DoH/DoT
# ==========================================
...
# ==========================================
# 9. SLOWDNS & BADVPN SETUP (FULLY FIXED & V2RAY READY)
# ==========================================
...
if [ -z "$skip_slowdns" ] && ( [ ! -s /etc/slowdns/dnstt-server ] || ! /etc/slowdns/dnstt-server -h >/dev/null 2>&1 ); then
    echo -e "${N_YELLOW}[!] Binary from GitHub failed. Trying fallback from dnstt.network...${NC}"
    if [[ "$ARCH" == "x86_64" ]]; then
...
if [ -z "$skip_slowdns" ]; then
    ...
    # Adjusted port from 109 to 81 to allow V2Ray + SSH Multiplexing via Nginx
...
ExecStart=/etc/slowdns/dnstt-server -udp 0.0.0.0:53 -privkey-file /etc/slowdns/server.key ${MY_NSDOMAIN} 127.0.0.1:81
Restart=on-failure
RestartSec=10
KillMode=process
LimitNOFILE=65536
...
END
    ...
    if [ ! -s /etc/slowdns/server.key ] || [ ! -s /etc/slowdns/server.pub ]; then
        echo -e "${N_YELLOW}[!] Keys missing, generating fresh ones...${NC}"
        cd /etc/slowdns
    ...
    echo -e "${N_YELLOW}[!] SlowDNS setup skipped due to missing binary or unsupported architecture.${NC}"
    cat > /etc/systemd/system/slowdns.service << END
[Unit]
    ...
    while true; do
        ...
        case $dt_opt in
            ...
            2|02)
                ...
                sed -i "s/server.key .*/server.key $newns 127.0.0.1:81/g" /etc/systemd/system/slowdns.service
                systemctl daemon-reload
...
settings_menu() {
    while true; do
        ...
        echo -e "  [11] ${WHITE}External TLS Handshake${NC}     [12] ${WHITE}Verbose WebSocket Test${NC}"
        echo -e "  [13] ${WHITE}WebSocket Echo Test${NC}"
        echo -e "${N_PINK}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        ...
        case $set_opt in
            ...
            12)
                clear
                echo -e "${N_CYAN} ❖ VERBOSE WEBSOCKET TEST (full headers) ${NC}"
                DOMAIN=$(cat /etc/xray/domain)
            ...
            13)
                clear
                echo -e "${N_CYAN} ❖ WEBSOCKET ECHO TEST (websocat) ${NC}"
                DOMAIN=$(cat /etc/xray/domain)
...
if systemctl is-active --quiet openvpn-server@server; then
    echo -e " ${N_GREEN}✔ OpenVPN is running${NC}"
else
...
if systemctl is-active --quiet slowdns; then
    ...
    if ss -tuln | grep -q ':53\s'; then
        echo -e " ${N_GREEN}✔ Port 53 is listening (UDP)${NC}"
    else
