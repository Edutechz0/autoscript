...
# =========================================================================================
# EDUFWESH PREMIUM AUTO-INSTALLER (ENTERPRISE NEON EDITION v4.7.1)
# =========================================================================================
# Core Features:
# - Enterprise Neon UI & ASCII Branding, Xray Sniffing, Anti-Abuse
...
# - FIXED: SSL Renewal Port 80 Clash (HAProxy/WS-ePro suspension added to Menu)
# - FIXED: HAProxy bypass for aggressive NapsternetV/HTTP Injector stacked payloads
# - FIXED: Bulletproof Xray XTLS-Reality Private Key string extraction
# - FIXED: Ultra-Accurate UID-based AutoKill Daemon for precise SSH multi-login drops
# - UPGRADED: VLESS HTTPUpgrade Support & Expanded NTLS Ports (2052, 8080)
# =========================================================================================
...
# =========================================================================================
# 0. HARDWARE ID & MONGODB LICENSE VERIFICATION
# =========================================================================================
...
# =========================================================================================
# 0.5. QUICK RESTORE MANAGER
# =========================================================================================
echo -e " ${N_YELLOW}[!] Choose Installation Mode:${NC}"
echo -e "  [1] Fresh Install (New Setup & Configuration)"
...
if [ "$INSTALL_MODE" == "2" ]; then
    ...
    if [ -f "$bpath" ]; then
        ...
        systemctl restart xray dropbear ssh stunnel4 nginx haproxy slowdns hysteria-server@config openvpn-server@server-udp openvpn-server@server-tcp badvpn-udpgw noobzvpns 2>/dev/null
...
mkdir -p /etc/xray /etc/slowdns /etc/edufwesh /etc/udp /etc/openvpn /etc/hysteria /var/log/xray /etc/stunnel /var/www/html/ovpn /etc/noobzvpns

touch /etc/edufwesh/xray-clients.txt /etc/edufwesh/trial_users.txt /etc/edufwesh/user_limits.txt
...
if [ -n "$MY_CDN" ]; then echo "$MY_CDN" > /etc/edufwesh/cdn_domain; fi
...
if /root/.acme.sh/acme.sh --issue -d ${MY_DOMAIN} --standalone --keylength ec-256 --force; then
    /root/.acme.sh/acme.sh --install-cert -d ${MY_DOMAIN} --ecc --fullchain-file /etc/stunnel/stunnel.crt --key-file /etc/stunnel/stunnel.key
...
# =========================================================================================
# 5. WS-EPRO (PERMISSIVE NAPSTERNET PAYLOAD ENGINE)
# =========================================================================================
echo -e "${N_CYAN}[+] Installing Universal Permissive WS-ePro Engine...${NC}"
...
def handle_client(client_socket):
    try:
        ...
        # Extremely permissive logic: If it resembles HTTP in any way, grant switching protocols
        if "HTTP/" in req_str or "GET " in req_str or "POST " in req_str or "websocket" in req_str.lower():
            response = "HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\n\r\n"
            client_socket.send(response.encode())
            
            # Forward any remaining payload body to SSH immediately
            ssh_index = request.find(b'SSH-')
            if ssh_index != -1:
                ssh_socket.sendall(request[ssh_index:])
            else:
                parts = request.split(b'\r\n\r\n')
                if len(parts) > 1 and len(parts[-1]) > 0:
                    ssh_socket.sendall(parts[-1])
        else:
...
[Unit]
Description=Universal Permissive WS-ePro Proxy Service
...
# =========================================================================================
# 6. XRAY CORE (VLESS, VMESS, TROJAN, SS + SOCKS5 + HTTPUpgrade)
# =========================================================================================
echo -e "${N_CYAN}[+] Installing & Optimizing Xray Core with DoH/DoT & HTTPUpgrade...${NC}"

bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install > /dev/null 2>&1

# FIXED: Bulletproof Reality Key Extraction
REALITY_KEYS=$(/usr/local/bin/xray x25519)
PRI_KEY=$(echo "$REALITY_KEYS" | grep -i "Private" | sed 's/.*:[ \t]*//' | tr -d '\r ')
PUB_KEY=$(echo "$REALITY_KEYS" | grep -i "Public" | sed 's/.*:[ \t]*//' | tr -d '\r ')
...
# =========================================================================================
# 6.6 AUTOMATED GHOST CLEANER & AUTOKILL DAEMON (PGREP UPGRADE)
# =========================================================================================
    ...
    grep "^${PROTOCOL} " /etc/edufwesh/xray-clients.txt | while read p deluser e i; do
        if [[ $(date +%s) -gt $(date -d "$e" +%s) ]]; then 
            if [ "$PROTOCOL" == "vmess" ]; then
                ...
                jq "del(.inbounds[0].settings.clients[] | select(.email == \"$deluser\")) | del(.inbounds[4].settings.clients[] | select(.email == \"$deluser\")) | del(.inbounds[7].settings.clients[] | select(.email == \"$deluser\")) | del(.inbounds[10].settings.clients[] | select(.email == \"$deluser\")) | del(.inbounds[11].settings.clients[] | select(.email == \"$deluser\"))" /usr/local/etc/xray/config.json > /tmp/xray.json
...
# =========================================================================================
# 7. NGINX & HAPROXY MULTIPLEXER (FIXED ACL BYPASS FOR AGGRESSIVE PAYLOADS)
# =========================================================================================
...
frontend http_front
    ...
    # FIXED: Strict Xray Routing. Any payload not matching these exact paths instantly drops to ws-epro bypass
    acl is_v2ray req.payload(0,0) -m sub /vmess
    acl is_v2ray req.payload(0,0) -m sub /vless
    acl is_v2ray req.payload(0,0) -m sub /vless-hu
...
# =========================================================================================
echo -e "${N_CYAN}[+] Setting up Dual OpenVPN (TCP/UDP)...${NC}"
...
echo -e "${N_CYAN}[+] Installing NoobzVPN v3.3.1 Daemon...${NC}"
...
cat > /etc/noobzvpns/config.toml << END
# NoobzVPNs TOML Configuration
tcp_std = ["8081"]
tcp_ssl = ["9443"]
cert = "/etc/stunnel/stunnel.pem"
key = "/etc/stunnel/stunnel.pem"
...
systemctl daemon-reload
(sleep 5 && systemctl enable --now noobzvpns.service > /dev/null 2>&1) &
# =========================================================================================
...
# =========================================================================================
echo -e "${N_CYAN}[+] Compiling Master Menu System...${NC}"
cat > /usr/bin/menu << 'END'
#!/bin/bash

# --- SUDO ENFORCER ---
if [ "${EUID}" -ne 0 ]; then
    echo -e "\e[1;31m[!] CRITICAL: You must run this menu as root! Type 'sudo su' first.\e[0m"
    exit 1
fi
...
# Standardized 85-Character Borders for sub-menus
B_THIN="━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ...
    while true; do
        ...
        case $dt_opt in
            ...
            2|02)
                ...
                ;;
            3|03)
                # FIXED: SSL Rate Limit / Port Clash bypass
                echo -e "${N_YELLOW} Regenerating Let's Encrypt SSL via Acme.sh...${NC}"
                # Stop all services that might be holding Port 80
                systemctl stop haproxy nginx ws-epro >/dev/null 2>&1
                fuser -k 80/tcp >/dev/null 2>&1
                
                if /root/.acme.sh/acme.sh --renew -d ${DOMAIN} --force --standalone; then
                    /root/.acme.sh/acme.sh --install-cert -d ${DOMAIN} --ecc \
                        --fullchain-file /etc/stunnel/stunnel.crt \
                        --key-file /etc/stunnel/stunnel.key
                    cat /etc/stunnel/stunnel.crt /etc/stunnel/stunnel.key > /etc/stunnel/stunnel.pem
                    echo -e "${N_GREEN} Certificate renewed successfully!${NC}"
                else
                    echo -e "${RED} Certificate renewal failed! You may be rate-limited by Let's Encrypt.${NC}"
                fi
                
                # Restart everything
                systemctl restart stunnel4 ws-epro nginx haproxy
                echo -e "${N_GREEN} Services restarted!${NC}"
                sleep 3
                ;;
            4|04)
    ...
    while true; do
        ...
        case $b_opt in
            ...
            4|04)
                ...
                if [ -f "$bpath" ]; then
                    tar -xzf "$bpath" -C /
                    systemctl restart xray dropbear ssh slowdns noobzvpns
    ...
    while true; do
        ...
        case $set_opt in
            ...
            2|02) 
                ...
                echo -e "  - HAProxy           : 80, 443, 2052, 8080"
                echo -e "  - Stunnel4          : 447, 777"
                echo -e "  - Xray WS           : 443 (TLS), 80/2052/8080 (NTLS)"
                echo -e "  - Xray HTTPUpgrade  : 443 (TLS), 80 (NTLS)"
                ...
                echo -e "  - NoobzVPN v3       : 8081 (STD), 9443 (SSL)"
            ...
            5|05)
                clear
                systemctl restart ssh dropbear stunnel4 ws-epro xray nginx haproxy slowdns badvpn-udpgw openvpn-server@server-udp openvpn-server@server-tcp noobzvpns hysteria-server@config
                if systemctl is-active --quiet edufwesh-bot; then 
                    systemctl restart edufwesh-bot
                fi
            ...
            9|09)
                ...
                echo ""
                for path in vmess vless vless-hu trojan ss; do
                    echo -n " $path: "
                ...
                declare -A port_map=( ["vless"]=10001 ["vmess"]=10002 ["trojan"]=10003 ["ss"]=10004 ["vless-hu"]=10010 )
                for path in vmess vless vless-hu trojan ss; do
                    port=${port_map[$path]}
                ...
                for port in 10001 10002 10003 10004 10005 10006 10010; do
    ...
    while true; do
        ...
        if [[ "$x_opt" == "1" || "$x_opt" == "01" || "$x_opt" == "2" || "$x_opt" == "02" ]]; then
            ...
            if [ "$PROTOCOL" == "vmess" ]; then
                ...
                jq ".inbounds[0].settings.clients += [{\"id\": \"${UUID}\", \"email\": \"${user}\"}] | .inbounds[4].settings.clients += [{\"id\": \"${UUID}\", \"email\": \"${user}\"}] | .inbounds[7].settings.clients += [{\"id\": \"${UUID}\", \"flow\": \"xtls-rprx-vision\", \"email\": \"${user}\"}] | .inbounds[10].settings.clients += [{\"id\": \"${UUID}\", \"email\": \"${user}\"}] | .inbounds[11].settings.clients += [{\"id\": \"${UUID}\", \"email\": \"${user}\"}]" /usr/local/etc/xray/config.json > /tmp/xray.json
            elif [ "$PROTOCOL" == "trojan" ]; then
            ...
            if [ "$PROTOCOL" == "vmess" ]; then
                echo -e "Port NTLS Extra: ${WHITE}2052${NC}"
                ...
                echo -e "Port NTLS Extra: ${WHITE}8080${NC}"
                echo -e "Port gRPC      : ${WHITE}443${NC}"
                echo -e "id             : ${WHITE}${UUID}${NC}"
                echo -e "encryption     : ${WHITE}none${NC}"
                echo -e "Network        : ${WHITE}ws, grpc, tcp (Reality), quic, httpupgrade${NC}"
                echo -e "Path           : ${WHITE}/vless (WS), /vless-hu (HTTPUpgrade)${NC}"
            elif [ "$PROTOCOL" == "trojan" ]; then
...
EOF
                ...
                VM_NTLS_80=$(cat <<EOF | base64 -w 0
{"v":"2","ps":"${user} (NTLS-80)","add":"${DOMAIN}","port":"80","id":"${UUID}","aid":"0","net":"ws","path":"/vmess","type":"none","host":"${DOMAIN}","tls":"none"}
EOF
)
                VM_NTLS_2052=$(cat <<EOF | base64 -w 0
{"v":"2","ps":"${user} (NTLS-2052)","add":"${DOMAIN}","port":"2052","id":"${UUID}","aid":"0","net":"ws","path":"/vmess","type":"none","host":"${DOMAIN}","tls":"none"}
EOF
)
...
EOF
                ...
                echo -e "Link NTLS 80   : ${WHITE}vmess://${VM_NTLS_80}${NC}"
                echo -e "Link NTLS 2052 : ${WHITE}vmess://${VM_NTLS_2052}${NC}"
...
EOF
                ...
                VL_NTLS_80="vless://${UUID}@${DOMAIN}:80?path=%2Fvless&security=none&encryption=none&type=ws&host=${DOMAIN}#${user}-NTLS-80"
                VL_NTLS_8080="vless://${UUID}@${DOMAIN}:8080?path=%2Fvless&security=none&encryption=none&type=ws&host=${DOMAIN}#${user}-NTLS-8080"
                VL_HU_TLS="vless://${UUID}@${DOMAIN}:443?path=%2Fvless-hu&security=tls&encryption=none&type=httpupgrade&host=${DOMAIN}&sni=${DOMAIN}#${user}-HU-TLS"
                VL_HU_NTLS="vless://${UUID}@${DOMAIN}:80?path=%2Fvless-hu&security=none&encryption=none&type=httpupgrade&host=${DOMAIN}#${user}-HU-NTLS"
                
                echo -e "Link TLS       : ${WHITE}${VL_TLS}${NC}"
                echo -e "Link NTLS 80   : ${WHITE}${VL_NTLS_80}${NC}"
                echo -e "Link NTLS 8080 : ${WHITE}${VL_NTLS_8080}${NC}"
                echo -e "Link HU TLS    : ${WHITE}${VL_HU_TLS}${NC}"
                echo -e "Link HU NTLS 80: ${WHITE}${VL_HU_NTLS}${NC}"
...
# =========================================================================================
# 11. MASTER DASHBOARD MENU SYSTEM (ENTERPRISE NEON + ASCII BRANDING)
# =========================================================================================
while true; do
    DOMAIN=$(cat /etc/xray/domain 2>/dev/null)
    NSDOM=$(cat /etc/slowdns/nsdomain 2>/dev/null)
    ...
    # Clean Enterprise Neon Color Palette
    E_CYAN='\e[38;5;87m'
    E_PINK='\e[38;5;213m'
    E_PURP='\e[38;5;141m'
    E_BLUE='\e[38;5;39m'
    ...
    clear
    echo -e "${E_PURP}╭──────────────────────────────────────────────────────────────────────────────────╮${NC}"
    echo -e "${E_PURP}│${NC}  ${E_CYAN}███████╗██████╗ ██╗   ██╗███████╗██╗    ██╗███████╗███████╗██╗  ██╗${NC}         ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}  ${E_CYAN}██╔════╝██╔══██╗██║   ██║██╔════╝██║    ██║██╔════╝██╔════╝██║  ██║${NC}         ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}  ${E_CYAN}█████╗  ██║  ██║██║   ██║█████╗  ██║ █╗ ██║███████╗███████╗███████║${NC}         ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}  ${E_CYAN}██╔══╝  ██║  ██║██║   ██║██╔══╝  ██║███╗██║╚════██║╚════██║██╔══██║${NC}         ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}  ${E_CYAN}███████╗██████╔╝╚██████╔╝██║     ╚███╔███╔╝███████║███████║██║  ██║${NC}         ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}  ${E_CYAN}╚══════╝╚═════╝  ╚═════╝ ╚═╝      ╚══╝╚══╝ ╚══════╝╚══════╝╚═╝  ╚═╝${NC}         ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}                      ${E_GRAY}Build v4.7.1 |  Telegram: @EDUFWESH3${NC}                        ${E_PURP}│${NC}"
    echo -e "${E_PURP}├──────────────────────────────────────────────────────────────────────────────────┤${NC}"
    echo -e "${E_PURP}│${NC} ${E_PINK}>> SYSTEM NODE${NC}                                                                     ${E_PURP}│${NC}"
    printf "${E_PURP}│${NC}  ${E_GRAY}%-12s${NC} : ${WHITE}%-22s${NC} ${E_GRAY}%-12s${NC} : ${WHITE}%-25s${NC} ${E_PURP}│${NC}\n" "OS" "${OS_INFO:0:22}" "Uptime" "$UPTIME"
    printf "${E_PURP}│${NC}  ${E_GRAY}%-12s${NC} : ${WHITE}%-22s${NC} ${E_GRAY}%-12s${NC} : ${WHITE}%-25s${NC} ${E_PURP}│${NC}\n" "Domain" "${DOMAIN:0:22}" "IP Address" "$IPVPS"
    printf "${E_PURP}│${NC}  ${E_GRAY}%-12s${NC} : ${WHITE}%-22s${NC} ${E_GRAY}%-12s${NC} : ${WHITE}%-25s${NC} ${E_PURP}│${NC}\n" "NS Domain" "${NSDOM:0:22}" "RAM Load" "${RAM_USED}MB / ${RAM_TOTAL}MB"
    printf "${E_PURP}│${NC}  ${E_GRAY}%-12s${NC} : ${WHITE}%-22s${NC} ${E_GRAY}%-12s${NC} : ${WHITE}%-25s${NC} ${E_PURP}│${NC}\n" "Daily Traffic" "$BW_TODAY" "Monthly" "$BW_MONTH"
    echo -e "${E_PURP}├──────────────────────────────────────────────────────────────────────────────────┤${NC}"
    echo -e "${E_PURP}│${NC} ${E_PINK}>> PROTOCOL REGISTRY${NC}                                                               ${E_PURP}│${NC}"
    printf "${E_PURP}│${NC}    ${E_CYAN}%-12s %-12s %-12s %-12s %-12s${NC}          ${E_PURP}│${NC}\n" "SSH/WS" "VMESS" "VLESS" "TROJAN" "SS-2022"
    printf "${E_PURP}│${NC}    ${WHITE}%-12s %-12s %-12s %-12s %-12s${NC}          ${E_PURP}│${NC}\n" "[$SSH_C]" "[$VMS_C]" "[$VLS_C]" "[$TRJ_C]" "[$SS_C]"
    echo -e "${E_PURP}├──────────────────────────────────────────────────────────────────────────────────┤${NC}"
    echo -e "${E_PURP}│${NC} ${E_PINK}>> CORE DAEMONS${NC}                                                                    ${E_PURP}│${NC}"
    printf "${E_PURP}│${NC}  ${WHITE}%-11s${NC} : %-16b ${WHITE}%-11s${NC} : %-16b ${WHITE}%-10s${NC} : %-15b ${E_PURP}│${NC}\n" "SSH" "$(check_run_ent ssh)" "Nginx" "$(check_run_ent nginx)" "Xray" "$(check_run_ent xray)"
    printf "${E_PURP}│${NC}  ${WHITE}%-11s${NC} : %-16b ${WHITE}%-11s${NC} : %-16b ${WHITE}%-10s${NC} : %-15b ${E_PURP}│${NC}\n" "WS-ePro" "$(check_run_ent ws-epro)" "HAProxy" "$(check_run_ent haproxy)" "SlowDNS" "$(check_run_ent slowdns)"
    printf "${E_PURP}│${NC}  ${WHITE}%-11s${NC} : %-16b ${WHITE}%-11s${NC} : %-16b ${WHITE}%-10s${NC} : %-15b ${E_PURP}│${NC}\n" "OVPN TCP" "$(check_run_ent openvpn-server@server-tcp)" "OVPN UDP" "$(check_run_ent openvpn-server@server-udp)" "Noobz v3" "$(check_run_ent noobzvpns)"
    printf "${E_PURP}│${NC}  ${WHITE}%-11s${NC} : %-16b ${WHITE}%-11s${NC} : %-16b ${WHITE}%-10s${NC} : %-15b ${E_PURP}│${NC}\n" "AutoKill" "$(check_run_ent autokill)" "TG Bot" "$(check_run_ent edufwesh-bot)" "Dropbear" "$(check_run_ent dropbear)"
    echo -e "${E_PURP}├──────────────────────────────────────────────────────────────────────────────────┤${NC}"
    echo -e "${E_PURP}│${NC} ${E_BLUE}✦ CLIENT MANAGERS${NC}                            ${E_BLUE}✦ ADMINISTRATION & TOOLS${NC}            ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}                                                                                  ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}  [01] ${WHITE}SSH/WS Manager${NC}                          [08] ${WHITE}Domain & SSL Manager${NC}          ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}  [02] ${WHITE}VMess Manager${NC}                           [09] ${WHITE}Server Settings Hub${NC}           ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}  [03] ${WHITE}VLess Manager${NC}                           [10] ${WHITE}Check Online Users${NC}            ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}  [04] ${WHITE}Trojan Manager${NC}                          [11] ${WHITE}OpenVPN Profiles${NC}              ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}  [05] ${WHITE}Xray gRPC Manager${NC}                       [12] ${WHITE}Check Services Detail${NC}         ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}  [06] ${WHITE}Shadowsocks Manager${NC}                     [13] ${WHITE}Change SSH Banner${NC}             ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}                                                                                  ${E_PURP}│${NC}"
    echo -e "${E_PURP}│${NC}                                        [00] ${RED}Exit System${NC}                          ${E_PURP}│${NC}"
    echo -e "${E_PURP}╰──────────────────────────────────────────────────────────────────────────────────╯${NC}"
    ...
    case $opt in
        ...
        5|05) 
            clear
            echo -e "${E_PURP}╭─────────────────────────────────────────────────────╮${NC}"
            echo -e "${E_PURP}│${NC}                 ${E_CYAN}XRAY gRPC MANAGER${NC}                   ${E_PURP}│${NC}"
            echo -e "${E_PURP}╰─────────────────────────────────────────────────────╯${NC}"
        ...
        13)
            clear
            echo -e "${E_CYAN} ❖ CHANGE SSH BANNER ❖ ${NC}"
            echo -e "${YELLOW} Opening /etc/issue.net in nano editor...${NC}"
            ...
            echo -e "\n ${E_GREEN}Banner successfully updated and applied!${NC}"
            read -n 1 -s -r -p "Press any key to return..."
            ;;
        8|08) domain_tools ;;
        9|09) settings_menu ;;
        ...
        12)
            clear
            echo -e "${E_PURP}╭──────────────────────────────────────────────────────────────────────────╮${NC}"
            echo -e "${E_PURP}│${NC}                         ${E_CYAN}DETAILED SERVICE STATUS${NC}                          ${E_PURP}│${NC}"
            echo -e "${E_PURP}╰──────────────────────────────────────────────────────────────────────────╯${NC}"
            ...
            printf "    %-30s : %b\n" "NoobzVPN v3 Server" "$(check_run noobzvpns)"
...
if systemctl is-active --quiet noobzvpns; then 
    echo -e " ${N_GREEN}✔ NoobzVPN v3 is running${NC}"
fi
