...
# Bughost & CDN bypass support for Xray configs
# ==========================================================
...
# ==========================================
# 10. MASTER DASHBOARD MENU SYSTEM (with bughost support)
# ==========================================
    ...
    while true; do
        ...
        case $dt_opt in
            ...
            3|03)
                ...
                ;;
            4|04)
                echo -e "${YELLOW} Regenerating config links for all existing Xray users with new domain...${NC}"
                echo -e "${GREEN} Note: Users must re-copy their config links from the respective menus.${NC}"
                echo -e "${GREEN} All new account creations will use the new domain automatically.${NC}"
                sleep 3
                ;;
            *)
...
settings_menu() {
    while true; do
        ...
        echo -e "  [10] Show Payload Examples (HTTP Injector / Napsternet)"
        echo -e "  [11] Test External TLS Handshake (openssl)"
        echo -e "  [12] Verbose WebSocket Test (full headers)"
        ...
        case $set_opt in
            ...
            9|09)
                clear
                echo -e "${CYAN}┌─────────────────────────────────────────────────────┐${NC}"
                echo -e "${CYAN}                 XRAY CONNECTIVITY TEST               ${NC}"
                echo -e "${CYAN}└─────────────────────────────────────────────────────┘${NC}"
                ...
                echo -e "\n${YELLOW}Testing Xray directly on localhost (bypassing proxy):${NC}"
                ...
                echo -e ""
                echo -e "${YELLOW}Xray local ports status:${NC}"
            ...
            10)
                clear
                echo -e "${CYAN}┌─────────────────────────────────────────────────────┐${NC}"
                echo -e "${CYAN}          PAYLOAD EXAMPLES FOR TUNNEL APPS            ${NC}"
                echo -e "${CYAN}└─────────────────────────────────────────────────────┘${NC}"
                DOMAIN=$(cat /etc/xray/domain)
                echo -e "${YELLOW}► HTTP Injector / Napsternet (VMESS/VLESS/Trojan)${NC}"
                echo -e "  Method: WebSocket"
                echo -e "  Host: ${DOMAIN} (or your bughost if using custom SNI)"
                echo -e "  Port: 443"
                echo -e "  SNI: ${DOMAIN} (or bughost for zero-rated)"
                echo -e "  Path: /vmess   (or /vless, /trojan, /ss)"
                echo -e "  TLS: Enable"
                echo -e "  (If your app has 'Skip Certificate Verification', enable it.)"
                echo -e ""
                echo -e "${YELLOW}► SSH over WebSocket (using WS-ePro)${NC}"
                echo -e "  Host: ${DOMAIN}"
                echo -e "  Port: 443"
                echo -e "  SNI: ${DOMAIN}"
                echo -e "  Path: / (root)"
                ...
                echo -e ""
                echo -e "${YELLOW}► SlowDNS (DNS tunnel)${NC}"
            ...
            11)
                clear
                echo -e "${CYAN}┌─────────────────────────────────────────────────────┐${NC}"
                echo -e "${CYAN}           EXTERNAL TLS HANDSHAKE TEST               ${NC}"
                echo -e "${CYAN}└─────────────────────────────────────────────────────┘${NC}"
                DOMAIN=$(cat /etc/xray/domain)
                echo -e " Testing TLS handshake to ${DOMAIN}:443 ..."
                echo -e ""
                echo -e "${YELLOW}Running openssl s_client...${NC}"
            ...
            12)
                clear
                echo -e "${CYAN}┌─────────────────────────────────────────────────────┐${NC}"
                echo -e "${CYAN}           VERBOSE WEBSOCKET TEST (full headers)     ${NC}"
                echo -e "${CYAN}└─────────────────────────────────────────────────────┘${NC}"
                DOMAIN=$(cat /etc/xray/domain)
            ...
            13)
                clear
                echo -e "${CYAN}┌─────────────────────────────────────────────────────┐${NC}"
                echo -e "${CYAN}           WEBSOCKET ECHO TEST (websocat)            ${NC}"
                echo -e "${CYAN}└─────────────────────────────────────────────────────┘${NC}"
                DOMAIN=$(cat /etc/xray/domain)
...
# ----- XRAY MANAGER FUNCTION (with bughost support) -----
xray_menu() {
    PROTOCOL=$1
    while true; do
        DOMAIN=$(cat /etc/xray/domain 2>/dev/null)
        ...
        if [[ "$x_opt" == "1" || "$x_opt" == "01" || "$x_opt" == "2" || "$x_opt" == "02" ]]; then
            ...
            # Optional bughost
            read -p " Do you want to add a bughost (custom SNI) for free browsing? (y/n): " add_bughost
            bughost=""
            if [[ "$add_bughost" == "y" || "$add_bughost" == "Y" ]]; then
                read -p " Enter bughost domain (e.g., screenclass.com): " bughost
            fi
            ...
            # Generate normal configs (using real domain)
            if [ "$PROTOCOL" == "vmess" ]; then
...
EOF
                ...
                echo -e " Normal configs (using your domain):"
                echo -e " Link TLS : ${WHITE}vmess://${VM_TLS}${NC}"
                echo -e " Link NTLS: ${WHITE}vmess://${VM_NTLS}${NC}"
                echo -e " Link gRPC: ${WHITE}vmess://${VM_GRPC}${NC}"
                
                if [ -n "$bughost" ]; then
                    VM_TLS_BUG=$(cat <<EOF | base64 -w 0
{"v":"2","ps":"${user} (bughost: ${bughost})","add":"${bughost}","port":"443","id":"${UUID}","aid":"0","net":"ws","path":"/vmess","type":"none","host":"${DOMAIN}","tls":"tls"}
EOF
)
                    VM_NTLS_BUG=$(cat <<EOF | base64 -w 0
{"v":"2","ps":"${user} (bughost: ${bughost})","add":"${bughost}","port":"80","id":"${UUID}","aid":"0","net":"ws","path":"/vmess","type":"none","host":"${DOMAIN}","tls":"none"}
EOF
)
                    VM_GRPC_BUG=$(cat <<EOF | base64 -w 0
{"v":"2","ps":"${user} (bughost: ${bughost})","add":"${bughost}","port":"443","id":"${UUID}","aid":"0","net":"grpc","path":"vmess-grpc","type":"none","host":"${DOMAIN}","tls":"tls"}
EOF
)
                    echo -e "${CYAN}───────────────────────────────────────────────────────${NC}"
                    echo -e " Bughost configs (SNI = ${bughost}, Host = ${DOMAIN}):"
                    echo -e " Link TLS : ${WHITE}vmess://${VM_TLS_BUG}${NC}"
                    echo -e " Link NTLS: ${WHITE}vmess://${VM_NTLS_BUG}${NC}"
                    echo -e " Link gRPC: ${WHITE}vmess://${VM_GRPC_BUG}${NC}"
                fi
                ...
                if [ -n "$bughost" ]; then
                    echo -e "${CYAN}───────────────────────────────────────────────────────${NC}"
                    echo -e " Bughost configs (SNI = ${bughost}, Host = ${DOMAIN}):"
                    echo -e " Link TLS : ${WHITE}vless://${UUID}@${bughost}:443?path=%2Fvless&security=tls&encryption=none&type=ws&sni=${bughost}&host=${DOMAIN}#${user}${NC}"
                    echo -e " Link NTLS: ${WHITE}vless://${UUID}@${bughost}:80?path=%2Fvless&security=none&encryption=none&type=ws&host=${DOMAIN}#${user}${NC}"
                    echo -e " Link gRPC: ${WHITE}vless://${UUID}@${bughost}:443?security=tls&encryption=none&type=grpc&serviceName=vless-grpc&sni=${bughost}&host=${DOMAIN}#${user}${NC}"
                fi
                ...
                if [ -n "$bughost" ]; then
                    echo -e "${CYAN}───────────────────────────────────────────────────────${NC}"
                    echo -e " Bughost configs (SNI = ${bughost}, Host = ${DOMAIN}):"
                    echo -e " Link TLS : ${WHITE}trojan://${user}@${bughost}:443?path=%2Ftrojan&security=tls&type=ws&sni=${bughost}&host=${DOMAIN}#${user}${NC}"
                    echo -e " Link NTLS: ${WHITE}trojan://${user}@${bughost}:80?path=%2Ftrojan&security=none&type=ws&host=${DOMAIN}#${user}${NC}"
                fi
                ...
                if [ -n "$bughost" ]; then
                    echo -e "${CYAN}───────────────────────────────────────────────────────${NC}"
                    echo -e " Bughost configs (SNI = ${bughost}, Host = ${DOMAIN}):"
                    echo -e " Link TLS : ${WHITE}ss://${SS_BASE}@${bughost}:443?path=%2Fss&security=tls&type=ws&sni=${bughost}&host=${DOMAIN}#${user}${NC}"
                fi
            fi
            
            echo -e "${CYAN}───────────────────────────────────────────────────────${NC}"
            echo -e " Note: For bughost configs, set the app's Address/SNI to the bughost"
            echo -e " and the Host/WS Host to your real domain (${DOMAIN})."
            read -n 1 -s -r -p "Press any key to return..."
