    ...
    while true; do
        ...
        case $set_opt in
            ...
            9|09)
                ...
                echo -e " Testing VMESS WebSocket on port 80 (HTTP)..."
                curl -s -o /dev/null -w "VMESS HTTP: %{http_code}\n" "http://${DOMAIN}/vmess" --header "Upgrade: websocket" --header "Connection: Upgrade"
                echo -e " Testing VLESS WebSocket on port 80 (HTTP)..."
                curl -s -o /dev/null -w "VLESS HTTP: %{http_code}\n" "http://${DOMAIN}/vless" --header "Upgrade: websocket" --header "Connection: Upgrade"
                echo -e " Testing Trojan WebSocket on port 80 (HTTP)..."
                curl -s -o /dev/null -w "Trojan HTTP: %{http_code}\n" "http://${DOMAIN}/trojan" --header "Upgrade: websocket" --header "Connection: Upgrade"
                echo -e "${CYAN}───────────────────────────────────────────────────────${NC}"
                echo -e " If you see '101' (Switching Protocols), the service is working."
                echo -e " If you see '400' or '404', check Xray and Nginx logs."
                echo -e ""
                echo -e "${YELLOW}Also checking Xray local ports:${NC}"
...
# ----- XRAY MANAGER FUNCTION (with live DOMAIN) -----
xray_menu() {
    PROTOCOL=$1
    while true; do
        DOMAIN=$(cat /etc/xray/domain 2>/dev/null)
...
if systemctl is-active --quiet slowdns; then
    ...
    # Also check if port 53 is listening
