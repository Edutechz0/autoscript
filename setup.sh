...
# Download dnstt-server from YOUR GitHub repository (replace with your repo)
ARCH=$(uname -m)
if [[ "$ARCH" == "x86_64" ]]; then
    DNSTT_URL="https://raw.githubusercontent.com/Edutechz0/autoscript/main/dnstt-server-amd64"
elif [[ "$ARCH" == "aarch64" ]]; then
    DNSTT_URL="https://raw.githubusercontent.com/Edutechz0/autoscript/main/dnstt-server-arm64"
else
...
if [ -z "$skip_slowdns" ]; then
    ...
    if [ ! -s /etc/slowdns/dnstt-server ]; then
        echo -e "${RED}[!] Failed to download dnstt-server. SlowDNS will be disabled.${NC}"
        skip_slowdns=1
    else
        echo -e "${GREEN}[+] dnstt-server binary ready.${NC}"
    fi
fi
