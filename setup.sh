...
# Neon Pro UI Design, Advanced Separation, Well Documented
# ==========================================================
...
# ==========================================
echo -e "\n${N_CYAN}[+] Freeing up Port 53 & Updating System...${NC}"
...
apt-get update -y
apt-get upgrade -y
apt-get install -y wget curl nano unzip ufw stunnel4 dropbear iptables nginx haproxy jq python3 uuid-runtime cron bc vnstat net-tools speedtest-cli openvpn easy-rsa psmisc
...
# 3. FIREWALL SETUP
# ==========================================
echo -e "${N_CYAN}[+] Configuring Firewall Ports...${NC}"
...
# ==========================================
echo -e "${N_CYAN}[+] Installing Advanced WS-ePro Engine...${NC}"
...
# ==========================================
echo -e "${N_CYAN}[+] Installing Xray Core...${NC}"
...
# ==========================================
echo -e "${N_CYAN}[+] Setting up OpenVPN & NoobzVPN Services...${NC}"
...
if [ -f "/etc/openvpn/easy-rsa/ta.key" ]; then
    cp /etc/openvpn/easy-rsa/ta.key /etc/openvpn/ta.key
...
ExecStartPre=/bin/bash -c "systemctl stop systemd-resolved || true"
ExecStartPre=/bin/bash -c "killall -9 systemd-resolved || true"
ExecStartPre=/bin/bash -c "fuser -k 53/udp || true"
ExecStartPre=/bin/bash -c "fuser -k 53/tcp || true"
...
# ==========================================
echo -e "${GREEN}[+] Compiling Neon Pro Master Menu System...${NC}"
...
domain_tools() {
    while true; do
        ...
        echo -e "    [03] ${WHITE}Force Renew SSL Certificate (Recommended)${NC}"
        echo -e "${N_PINK}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo -e "    [00] ${RED}Back to Main Menu${NC}"
        echo -e "${N_PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        read -p " Select menu : " dt_opt
        
        if [[ "$dt_opt" == "0" || "$dt_opt" == "00" ]]; then break; fi
...
settings_menu() {
    while true; do
        ...
        case $set_opt in
            1|01) clear; echo -e "${N_CYAN} ❖ SPEEDTEST RESULTS ${NC}"; speedtest-cli; read -n 1 -s -r -p "Press any key..." ;;
            2|02) 
                clear
                ...
                read -n 1 -s -r -p "Press any key to return..." ;;
            3|03) clear; read -p " Reboot every how many hours? (e.g., 12): " hr; echo "0 */$hr * * * root /sbin/reboot" > /etc/cron.d/auto_reboot; echo -e "${N_GREEN} Auto-reboot set to every $hr hours.${NC}"; sleep 2 ;;
            4|04) clear; uptime; echo ""; free -h; echo ""; df -h | grep '^/dev/'; read -n 1 -s -r -p "Press any key..." ;;
            5|05) clear; systemctl restart ssh dropbear stunnel4 ws-epro xray nginx haproxy slowdns badvpn-udpgw openvpn-server@server noobzvpnd; echo -e "${N_GREEN} All Core Services Restarted Successfully!${NC}"; sleep 2 ;;
            6|06) clear; vnstat; read -n 1 -s -r -p "Press any key..." ;;
            7|07)
                clear
                ...
                sleep 2 ;;
            8|08) clear; echo -e "${N_YELLOW} Downloading UDP Custom binary...${NC}"; wget -q -O /usr/bin/udp-custom "https://raw.githubusercontent.com/Edutechz0/autoscript/main/udp-custom" 2>/dev/null || echo -e "${RED} UDP Custom Binary not found!${NC}"; chmod +x /usr/bin/udp-custom 2>/dev/null; echo -e "${N_GREEN} UDP Setup completed.${NC}"; read -n 1 -s -r -p "Press any key..." ;;
            0|00) break ;;
        esac
    done
}
...
xray_menu() {
    ...
    while true; do
        ...
        if [[ "$x_opt" == "1" || "$x_opt" == "01" || "$x_opt" == "2" || "$x_opt" == "02" ]]; then
            clear
            echo -e "${N_CYAN} ❖ CREATE XRAY ${PROTOCOL^^} USER ${NC}"
            ...
            if [ "$PROTOCOL" == "trojan" ]; then echo -e " Password   : ${WHITE}${user}${NC}"; else echo -e " ID         : ${WHITE}${UUID}${NC}"; fi
            echo -e "${N_PINK}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
EOF
                ...
                echo -e " ${N_YELLOW}Link TLS :${NC} ${WHITE}vmess://${VM_TLS}${NC}"
                echo -e " ${N_YELLOW}Link NTLS:${NC} ${WHITE}vmess://${VM_NTLS}${NC}"
                echo -e " ${N_YELLOW}Link gRPC:${NC} ${WHITE}vmess://${VM_GRPC}${NC}"
            elif [ "$PROTOCOL" == "vless" ]; then
                echo -e " ${N_YELLOW}Link TLS :${NC} ${WHITE}vless://${UUID}@${DOMAIN}:443?path=%2Fvless&security=tls&encryption=none&type=ws#${user}${NC}"
                echo -e " ${N_YELLOW}Link NTLS:${NC} ${WHITE}vless://${UUID}@${DOMAIN}:80?path=%2Fvless&security=none&encryption=none&type=ws#${user}${NC}"
                echo -e " ${N_YELLOW}Link gRPC:${NC} ${WHITE}vless://${UUID}@${DOMAIN}:443?security=tls&encryption=none&type=grpc&serviceName=vless-grpc#${user}${NC}"
            elif [ "$PROTOCOL" == "trojan" ]; then
                echo -e " ${N_YELLOW}Link TLS :${NC} ${WHITE}trojan://${user}@${DOMAIN}:443?path=%2Ftrojan&security=tls&type=ws#${user}${NC}"
                echo -e " ${N_YELLOW}Link NTLS:${NC} ${WHITE}trojan://${user}@${DOMAIN}:80?path=%2Ftrojan&security=none&type=ws#${user}${NC}"
            elif [ "$PROTOCOL" == "shadowsocks" ]; then
                SS_BASE=$(echo -n "aes-128-gcm:${UUID}" | base64 -w 0)
                echo -e " ${N_YELLOW}Link TLS :${NC} ${WHITE}ss://${SS_BASE}@${DOMAIN}:443?path=%2Fss&security=tls&type=ws#${user}${NC}"
            fi
            echo -e "${N_PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            read -n 1 -s -r -p "Press any key to return..."
            ...
            if [ -f "/etc/edufwesh/xray-clients.txt" ]; then
                grep "^${PROTOCOL} " /etc/edufwesh/xray-clients.txt | while read p u e i; do
                    if [[ $(date +%s) -gt $(date -d "$e" +%s) ]]; then stat="${RED}Expired${NC}"; else stat="${N_GREEN}Active${NC}"; fi
                    printf "  %-15s %-15s %b\n" "$u" "$e" "$stat"
                done
            else
...
ssh_menu() {
    while true; do
        ...
        if [[ "$s_opt" == "1" || "$s_opt" == "01" || "$s_opt" == "2" || "$s_opt" == "02" ]]; then
            ...
            if [[ "$s_opt" == "2" || "$s_opt" == "02" ]]; then days=1; user="Trial-$user"; else read -p " Days Active : " days; fi
            ...
            for user in $(awk -F: '$3 >= 1000 && $1 != "nobody" {print $1}' /etc/passwd); do
                ...
                if [[ "$exp" != "never" ]]; then
                    if [[ $(date +%s) -gt $(date -d "$exp" +%s) ]]; then stat="${RED}Expired${NC}"; else stat="${N_GREEN}Active${NC}"; fi
                    printf "  %-15s %-15s %b\n" "$user" "$exp" "$stat"
                fi
            done
            echo -e "${N_PINK}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
clear
echo -e "${N_PURPLE}██████████████████████████████████████████████████████████████████████${NC}"
echo -e "${N_PURPLE}█${NC} ${N_CYAN}                  𝗘𝗱𝘂𝗳𝘄𝗲𝘀𝗵 𝗩𝗣𝗡 𝗠𝗔𝗡𝗔𝗚𝗘𝗥 v2.0                 ${NC} ${N_PURPLE}█${NC}"
...
printf "  ${WHITE}%-10s${NC}: %-15b ${WHITE}%-10s${NC}: %-15b ${WHITE}%-10s${NC}: %-15b\n" "DROPBEAR" "$(check_run dropbear)" "WS-EPRO" "$(check_run ws-epro)" "OPENVPN" "$(check_run openvpn-server@server)"
printf "  ${WHITE}%-10s${NC}: %-15b ${WHITE}%-10s${NC}: %-15b ${WHITE}%-10s${NC}: %-15b\n" "STUNNEL4" "$(check_run stunnel4)" "SLOWDNS" "$(check_run slowdns)" "HAPROXY" "$(check_run haproxy)"
echo -e ""
echo -e "${N_PURPLE}██████████████████████████████████████████████████████████████████████${NC}"
echo -e "${N_CYAN} ✦ PROTOCOL MANAGERS               ✦ SYSTEM & TOOLS ${NC}"
echo -e "${N_PINK} ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "  [01] ${WHITE}SSH Manager${NC}                 [07] ${WHITE}Domain & SSL Manager${NC}"
echo -e "  [02] ${WHITE}VMess Manager${NC}               [08] ${WHITE}Server Settings Hub${NC}"
echo -e "  [03] ${WHITE}VLess Manager${NC}               [09] ${WHITE}Check Online Users${NC}"
echo -e "  [04] ${WHITE}Trojan Manager${NC}              [10] ${WHITE}OpenVPN & NoobzVPN${NC}"
echo -e "  [05] ${WHITE}Xray gRPC Manager${NC}           [11] ${WHITE}Check Services Detail${NC}"
echo -e "  [06] ${WHITE}Shadowsocks Manager${NC}         [00] ${RED}Exit System${NC}"
echo -e "${N_PURPLE}██████████████████████████████████████████████████████████████████████${NC}"
...
case $opt in
    ...
    5|05) 
        clear
        echo -e "${N_PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo -e "${N_CYAN}                       XRAY gRPC MANAGER                             ${NC}"
        echo -e "${N_PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    ...
    11)
        clear
        echo -e "${N_PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo -e "${N_CYAN}                     DETAILED SERVICE STATUS                         ${NC}"
        echo -e "${N_PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
echo -e "\n${N_GREEN}======================================${NC}"
echo -e "${N_GREEN}INSTALLATION COMPLETE!${NC}"
echo -e "${N_GREEN}Type 'menu' to access the expanded Dashboard.${NC}"
echo -e "${N_GREEN}======================================${NC}"
