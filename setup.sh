...
# Fully Integrated: OpenVPN, NoobzVPN, Check Online, Domain Menu
# ==========================================================
...
apt-get update -y
apt-get upgrade -y
apt-get install -y wget curl nano unzip ufw stunnel4 dropbear iptables nginx haproxy jq python3 uuid-runtime cron bc vnstat net-tools speedtest-cli openvpn easy-rsa
...
# Hardcode UDP Port 53 routing to ensure SlowDNS receives traffic
...
# ==========================================
# 8. OPENVPN & NOOBZVPN SETUP (Fully Integrated)
# ==========================================
echo -e "${GREEN}[+] Setting up OpenVPN & NoobzVPN Services...${NC}"

# OpenVPN Basic Server Config Generation
...
# NoobzVPN Fetcher
wget -q -O /usr/bin/noobzvpnd "https://raw.githubusercontent.com/Edutechz0/autoscript/main/noobzvpnd" 2>/dev/null || echo -e "${YELLOW}NoobzVPN Binary not found in repo, creating stub.${NC}"
chmod +x /usr/bin/noobzvpnd 2>/dev/null
...
check_online() {
    ...
    # Extracts established connections on ports 22, 109, 143, 443
    ...
    # Parses the Xray access log for recent accepted connections
