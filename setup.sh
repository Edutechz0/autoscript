...
apt-get update -y
apt-get upgrade -y
apt-get install -y wget curl nano unzip ufw stunnel4 dropbear iptables nginx haproxy jq python3 uuid-runtime cron bc vnstat net-tools speedtest-cli openvpn easy-rsa psmisc iptables-persistent golang git
...
# Download dnstt-server – detect architecture
ARCH=$(uname -m)
case $ARCH in
    x86_64)  DNSTT_URL="https://github.com/iwakdusk/dnstt/releases/download/v1.0/dnstt-server-linux-amd64" ;;
    aarch64) DNSTT_URL="https://github.com/iwakdusk/dnstt/releases/download/v1.0/dnstt-server-linux-arm64" ;;
    *)       DNSTT_URL="https://github.com/iwakdusk/dnstt/raw/main/dnstt-server" ;;
esac

wget -q --timeout=15 -O /etc/slowdns/dnstt-server "$DNSTT_URL" || \
curl -s -o /etc/slowdns/dnstt-server "$DNSTT_URL" || \
wget -q --timeout=15 -O /etc/slowdns/dnstt-server "https://github.com/iwakdusk/dnstt/releases/download/v1.0/dnstt-server-linux-amd64"

chmod +x /etc/slowdns/dnstt-server

# Verify binary works
if ! /etc/slowdns/dnstt-server -h >/dev/null 2>&1; then
    echo -e "${RED}[!] DNSTT binary is broken. Trying to compile from source...${NC}"
    apt-get install -y golang git
    git clone https://github.com/iwakdusk/dnstt /tmp/dnstt
    cd /tmp/dnstt && go build -o dnstt-server ./cmd/server
    ...
    while true; do
        ...
        case $dt_opt in
            1|01) 
                ...
                # Force menu to reload domain on next redraw
                sleep 2
                ;;
            2|02)
            ...
            4|04)
                ...
                # This only re-displays the links; actual configs are generated on demand.
    ...
    while true; do
        ...
        case $set_opt in
            ...
            9|09)
                ...
                echo -e " Testing VMESS WebSocket TLS on port 443..."
                curl -sk -o /dev/null -w "VMESS TLS: %{http_code}\n" "https://${DOMAIN}/vmess" --header "Upgrade: websocket" --header "Connection: Upgrade"
                echo -e " Testing VLESS WebSocket TLS..."
                curl -sk -o /dev/null -w "VLESS TLS: %{http_code}\n" "https://${DOMAIN}/vless" --header "Upgrade: websocket" --header "Connection: Upgrade"
                echo -e " Testing Trojan WebSocket TLS..."
                curl -sk -o /dev/null -w "Trojan TLS: %{http_code}\n" "https://${DOMAIN}/trojan" --header "Upgrade: websocket" --header "Connection: Upgrade"
                echo -e "${CYAN}───────────────────────────────────────────────────────${NC}"
                echo -e " If you see '101' (Switching Protocols), the service is working."
                echo -e " If you see '000' or '404', check Xray and Nginx logs."
                read -n 1 -s -r -p "Press any key to return..."
                ;;
            10)
                clear
...
# ----- XRAY MANAGER FUNCTION (unchanged except using live DOMAIN) -----
xray_menu() {
    PROTOCOL=$1
    while true; do
        DOMAIN=$(cat /etc/xray/domain 2>/dev/null)
