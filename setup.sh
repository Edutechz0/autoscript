...
# ==========================================================
# EDUFWESH PREMIUM AUTO-INSTALLER (GOD-TIER FINAL)
# Fully Expanded Codebase, HAProxy, WS-ePro, Payload Gens
# ==========================================================
...
apt-get update -y
apt-get upgrade -y
apt-get install -y wget curl nano unzip ufw stunnel4 dropbear iptables nginx haproxy jq python3 uuid-runtime cron bc vnstat net-tools speedtest-cli openvpn
...
settings_menu() {
    while true; do
        ...
        case $set_opt in
            ...
            5|05)
                clear
                systemctl restart ssh dropbear stunnel4 ws-epro xray nginx haproxy slowdns badvpn-udpgw
...
echo -e "    [05] CHECK ONLINE          [00] EXIT SYSTEM"
echo -e "${CYAN}└─────────────────────────────────────────────────────┘${NC}"
echo -e "${CYAN}┌─────────────────────────────────────────────────────┐${NC}"
...
case $opt in
    ...
    5|05) 
        ...
        echo -e "${CYAN}└─────────────────────────────────────────────────────┘${NC}"
        echo -e "${YELLOW} Check Online feature pending integration...${NC}"
        echo -e "${CYAN}───────────────────────────────────────────────────────${NC}"
        read -n 1 -s -r -p "Press any key to return..."
        ;;
    7|07) 
