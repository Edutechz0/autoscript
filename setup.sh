...
# TCP Fallback Routing, Payload Bypass, 100% Uncompressed
# ==========================================================
...
# ==========================================
# 5. WS-EPRO (Python eProxy Engine)
# ==========================================
...
# ==========================================
# 7. NGINX & HAPROXY FALLBACK ROUTING
# ==========================================
echo -e "${GREEN}[+] Configuring HAProxy TCP Fallback Routing...${NC}"
rm -f /etc/nginx/sites-enabled/default

# Nginx handles ONLY legitimate internal Xray routes now
...
# HAProxy analyzes SNI and Path, routes to Nginx OR WS-ePro directly
...
frontend http_front
    ...
    # If payload contains valid Xray paths, send to Nginx
    ...
    # DEFAULT: Any other weird HTTP payload goes straight to WS-ePro (Bypassing Nginx 400 Error!)
...
frontend https_front
    ...
    # If SNI exactly matches your main VPS domain, it's Xray traffic. Send to Nginx.
    acl is_mydomain req_ssl_sni -i ${MY_DOMAIN}
    use_backend nginx_https if is_mydomain

    # DEFAULT: If it's a Bug SNI (like images.vodafone.co.uk), it's SSH-TLS-Payload. 
    # Send to Stunnel (447) to decrypt, which forwards to WS-ePro directly.
...
# Configure Nginx SSL listener for HAProxy backend routing
cat > /etc/nginx/sites-available/xray-ssl.conf << END
server {
...
END
ln -s /etc/nginx/sites-available/xray-ssl.conf /etc/nginx/sites-enabled/xray-ssl.conf
...
openvpn --genkey tls-auth ta.key 2>/dev/null || openvpn --genkey secret ta.key 2>/dev/null || /usr/sbin/openvpn --genkey secret /etc/openvpn/easy-rsa/ta.key 2>/dev/null
...
systemctl enable noobzvpnd.service > /dev/null 2>&1
...
echo -e "    [02] VMESS MANAGER [Menu]  [07] DOMAIN & SSL MANAGER"
echo -e "    [03] VLESS MANAGER [Menu]  [08] SERVER SETTINGS"
echo -e "    [04] TROJAN MANAGER[Menu]  [09] CHECK RUNNING"
echo -e "    [05] XRAY gRPC     [Menu]  [10] CHECK ONLINE USERS"
echo -e "                               [11] OPENVPN / NOOBZVPN"
